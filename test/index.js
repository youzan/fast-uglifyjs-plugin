...
describe('FastUglifyJsPlugin', function () {
    describe('firstly build without cache', function () {
        ...
        it('should create a output map', function () {
            var stats = fs.statSync(path.join(__dirname, 'outs/factorial.js.map'));
            assert.isTrue(stats.isFile());
        });
    ...
    describe('secondly build with cache', function () {
        it('should build', function (done) {
            webpack(config, function (err) {
                assert.isNull(err);
                done();
