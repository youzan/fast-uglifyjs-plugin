    ...
    compiler.plugin("compilation", function (compilation) {
        ...
        compilation.plugin("optimize-chunk-assets", function (chunks, callback) {
            ...
            files.forEach(function (file) {
                ...
                var curFilePath = cacheFolder + '/' + file.replace(compilation.hash, '').slice(0, -3) + '.' + fileHash + '.js';
                console.log(fileHash);
                console.log(file);
                var inputSourceMap, cachedContent;
                ...
                while (workerNum > 0) {
                    ...
                    worker.on('message', function (m) {
                        if (m.cmd === 'complete') {
                            ...
                            var name = file.replace(compilation.hash, '').slice(0, -3) + '.' + fileHash + '.js';
                            fs.writeFileSync(cacheFolder + '/' + name, asset.__UglifyJsPlugin._value);
                            completeNum++;
