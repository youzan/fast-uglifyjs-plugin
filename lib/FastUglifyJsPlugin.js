    ...
    compiler.plugin("compilation", function (compilation) {
        ...
        compilation.plugin("optimize-chunk-assets", function (chunks, callback) {
            ...
            files.forEach(function (file) {
                ...
                var curFile = filenameHash + '.' + fileHash;
                var inputSourceMap;
                if (isCache) {
                    if (options.sourceMap && uglifyCache.isContainCacheMap(curFile)) {
                        var cachedContent = uglifyCache.getCacheContentMap(curFile);
                        var content = JSON.parse(cachedContent);
                        asset.__UglifyJsPlugin = compilation.assets[file] = new SourceMapSource(content.stream, content.file, content.map, content.input, content.inputSourceMap);
                        // 记录未变化chunk
                        unChangedChunks.push(file);
                        return;
                    }
                    if (!options.sourceMap && uglifyCache.isContainCache(curFile)) {
                        var cachedContent = uglifyCache.getCacheContent(curFile);
                        var content = JSON.parse(cachedContent);
                        asset.__UglifyJsPlugin = compilation.assets[file] = new RawSource(content.stream);
                        // 记录未变化chunk
                        unChangedChunks.push(file);
                        return;
                    }
                    ...
                    worker.on('message', function (m) {
                        if (m.cmd === 'complete') {
                            ...
                            if (isCache) {
                                ...
                                var name = filenameHash + '.' + fileHash;
                                if (result.map) {
                                    name += '.map';
                                    cacheContent = JSON.stringify({
                                        ...
                                        input: task.input,
                                        inputSourceMap: task.inputSourceMap
                                    });
                                } else {
                                    cacheContent = JSON.stringify({stream: result.stream});
                                }
                                uglifyCache.setCacheContent(filenameHash, fileHash, name, cacheContent);
                            }
                            completeNum++;
